...
class scoreboard extends uvm_scoreboard;
	...
	task reference();
		`uvm_info("SCB",$sformatf("INP_TX-> reset=%b AWADDR=%d ,AWPROT=%b ,AWVALID=%b ,WDATA=%h ,WVALID=%b ,WSTRB=%b ,BREADY=%b ,ARADDR=%d ,ARPROT=%b ,ARVALID=%b ,RREADY=%b",
		...
		`uvm_info("OUT_MON", $sformatf("OUT_TX -> AWREADY:%b | WREADY:%0b | BRESP:%0b | BVALID:%0b | ARREADY:%b | RDATA:%0h | RRESP:%0b | RVALID:%0b  ",
		...
		if(!inp_tx.reset)begin
				    ...
				    if(waddr_done && wdata_done) begin
					         ...
					         if(addr > 32'h40)
							        BRESP=2'b11;
							         ...
							         BRESP=2'b00;
							         temp={ {8{wstrb[3]}},{8{wstrb[2]}},{8{wstrb[1]}},{8{wstrb[0]}} };
							         mem[addr>>2]=(mem[addr>>2] &(~temp)) | (w_data&temp);
							        end
					        end
		 ...
		 `uvm_info("SCB", $sformatf(
			       "EXP -> waddr_done=%b, wdata_done=%b, addr=%d, w_data=%h,  mem[addr]=%d | BRESP:%0b | BVALID:%0b |  RDATA:%0h | RRESP:%0b | RVALID:%0b  ",
